COMPONENT=$1 
ENV=$2
HOSTEDZONEID="Z05257022TR9OLEZRO8JM"
...
SG_ID="$(aws ec2 describe-security-groups  --filters Name=group-name,Values=learn_test_group | jq '.SecurityGroups[].GroupId' | sed -e 's/"//g')"       # b54-allow-all security group id
